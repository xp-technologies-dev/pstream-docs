...
description: Common issues and solutions for self-hosting P-Stream.
---
...
You should have at least 1 Worker registered, if you don't, you should [deploy a worker](/pstream-docs/proxy/deploy) and [set it up in the client](/pstream-docs/client/configuration#vite-cors-proxy-url).
...
If the backend is online and properly configured it should display the name and version of the backend. If the name and description of the test don't match your own instance, [make sure you have your backend URL set correctly.](/pstream-docs/client/configuration#vite-backend-url)

If the test gives you an error, your [backend URL configuration option](/pstream-docs/client/configuration#vite-backend-url) likely has a typo.
...
It is likely that you haven't installed the PWA version of P-Stream. Please read the [upgrade guide](/pstream-docs/client/upgrade) for more details on the matter.

## I'm getting SSL issues when using a hosted postgres database

You are most likely missing the [`postgres.ssl`](/pstream-docs/backend/configuration#postgres-ssl) variable on your backend, enable it and the connection should work.

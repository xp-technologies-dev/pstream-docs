---
title: Streaming
description: Different ways to enable P-Stream to stream movies and TV shows.
---
...
Self-hosting a proxy is an easy way to get faster streaming speeds, [we have a guide](/pstream-docs/proxy/deploy) that explains how to set up one for **free**.
...
<Steps>
1. Set up a proxy using one of our [guides](/pstream-docs/proxy/deploy), [though we recommend Docker](/pstream-docs/proxy/deploy#method-1-docker-recommended).
...
[`VITE_CORS_PROXY_URL`](/pstream-docs/client/configuration#vite-cors-proxy-url)
...
This is not the case with self-hosted clients, there is no proxy set up by default on self-hosted clients and you will need to [set up one yourself](/pstream-docs/proxy/deploy).

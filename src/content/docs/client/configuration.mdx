...
This is the **read** API key from TMDB to allow P-Stream to search for media. [Get one by following our guide](/client/tmdb).
...
**The same proxy as above**, but used to proxy streams. However, it **must** be hosted on a server that is NOT blocked by FlixHQ. See this [explainer](/proxy/introduction).
...
[The proxy workers will need to be configured to accept these captcha tokens](/proxy/configuration#turnstile-secret), otherwise it has no effect for security.

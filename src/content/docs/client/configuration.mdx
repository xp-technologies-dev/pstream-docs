---
title: Configuration
description: Configuration reference for the P-Stream client.
---
...
This is the **read** API key from TMDB to allow P-Stream to search for media. [Get one by following our guide](/pstream-docs/client/tmdb).
...
This is where you put proxy URLs. [Get some by following our guides](/pstream-docs/proxy/deploy).
...
**The same proxy as above**, but used to proxy streams. However, it **must** be hosted on a server that is NOT blocked by FlixHQ. See this [explainer](/pstream-docs/proxy/introduction).

If left empty, some streams, notably streams from FlixHQ, WILL NOT work.
...
If omitted, this will still show the proxy onboarding screen, just without a documentation link for the proxy.
...
[The proxy workers will need to be configured to accept these captcha tokens](/pstream-docs/proxy/configuration#turnstile-secret), otherwise it has no effect for security.

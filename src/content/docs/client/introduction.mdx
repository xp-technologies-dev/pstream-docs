---
title: Introduction
description: Introduction to the P-Stream client.
---
...
The client can be optionally ran as a [PWA](https://web.dev/explore/progressive-web-apps), which allows it to be installed on a mobile device. **This can be hard to do correctly and really hard if not almost impossible to reverse**, so it's generally not recommended to do so if you don't have experience hosting PWAs. If you understand the risks and still want to continue, then read more about it [here](/pstream-docs/self-hosting/about-pwa).

## Configuration

The client features various configuration options, some of which are required for the client to function. [If you are using Vercel to host the client](/pstream-docs/client/deploy#method-2-vercel-might-be-broken-idk), then the required variables are a necessary part of creating the site, if you're using another host, or hosting it for yourself, you'll need to set them up yourself.

You can view all of the configuration options on the [configurations page](/pstream-docs/client/configuration).
